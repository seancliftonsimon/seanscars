...
export function calculateBordaScores(ballots) {
  ...
  const results = Object.values(movieStats).map(movie => {
    ...
    const avgPointsPerViewer = movie.seenCount > 0 
      ? movie.totalPoints / movie.seenCount 
      : 0;
...
export function calculateUnderSeenAwards(ballots, threshold = 0.40) {
  const bestPictureResults = calculateBordaScores(ballots);
  const totalBallots = ballots.length;

  // Filter for under-seen movies (seenFraction <= threshold)
  const underSeenMovies = bestPictureResults.filter(movie => movie.seenFraction <= threshold);
    ...
    if (ballot.movies && Array.isArray(ballot.movies)) {
      ballot.movies.forEach(movie => {
        if (movie.underSeenRec && movie.id) {
          recommendationCounts[movie.id] = (recommendationCounts[movie.id] || 0) + 1;
        }
      });
    }
  });
    ...
    if (ballot.movies && Array.isArray(ballot.movies)) {
      ballot.movies.forEach(movie => {
        if (movie.favoriteScary && movie.id) {
          categories.favoriteScary[movie.id] = (categories.favoriteScary[movie.id] || 0) + 1;
        }
        if (movie.funniest && movie.id) {
          categories.funniest[movie.id] = (categories.funniest[movie.id] || 0) + 1;
        }
        if (movie.bestTimeAtMovies && movie.id) {
          categories.bestTimeAtMovies[movie.id] = (categories.bestTimeAtMovies[movie.id] || 0) + 1;
        }
      });
    }
  });
